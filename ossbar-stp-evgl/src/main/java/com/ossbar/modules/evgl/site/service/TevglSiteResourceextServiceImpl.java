...
public class TevglSiteResourceextServiceImpl implements TevglSiteResourceextService {
	...
	private void verifyForm(TsysResource menu){
		...
		if(StringUtils.isBlank(menu.getName())){
			throw new CreatorblueException("名称不能为空");
		}
			...
			if (list.size() > 0) {
				list.forEach(a -> {
					if (a.getName().equals(menu.getName())) {
						throw new CreatorblueException("名称已存在,同级节点不允许有相同的名称");
					}
				});
				return ;
			}
		...
		if(menu.getType() == 3){
			if(parentType == 4){
				throw new CreatorblueException("父级节点只能为站点类型或者为空");
			}
			return ;
		}
		//栏目
		if(menu.getType() == 4){
			if(parentType != 3 && parentType != 4 || parentType==-1){
				throw new CreatorblueException("父级节点只能为站点或者栏目类型");
			}
			return ;
		}
