...
public class TevglTchClassroomGroupServiceImpl implements TevglTchClassroomGroupService {
	...
	public R save(@RequestBody(required = false) TevglTchClassroomGroup tevglTchClassroomGroup) throws CreatorblueException {
	...
	public R update(@RequestBody(required = false) TevglTchClassroomGroup tevglTchClassroomGroup) throws CreatorblueException {
	    //ValidatorUtils.check(tevglTchClassroomGroup);
	 ...
	 */
	@Override
	public R setTraineeToGroup(JSONObject jsonObject, String loginUserId) throws CreatorblueException {
	...
	public R saveGroupInfo(@RequestBody JSONObject jsonObject, String loginUserId) {
		...
		String msg = "";
		List<String> inputListName = (List<String>) list.stream().map(a -> a.toString()).distinct().collect(Collectors.toList());
		// 等待入库的名称
		...
		if (groupList != null && groupList.size() > 0) {
			...
			waitingList = (List<String>) list.stream().distinct().map(a -> a.toString()).collect(Collectors.toList());
		}
		// 入库
	 ...
	 */
	@Override
	public R deleteClassroomGroup(String gpId, String loginUserId) throws CreatorblueException {
		if (StrUtils.isEmpty(gpId) || StrUtils.isEmpty(loginUserId)) {
			return R.error("必传参数为空");
		}
		TevglTchClassroomGroup group = tevglTchClassroomGroupMapper.selectObjectById(gpId);
