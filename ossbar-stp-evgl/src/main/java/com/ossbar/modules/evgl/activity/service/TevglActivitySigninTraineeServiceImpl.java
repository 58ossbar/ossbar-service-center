...
import com.ossbar.modules.common.DateTools;
...
public class TevglActivitySigninTraineeServiceImpl implements TevglActivitySigninTraineeService {
	...
	public R save(@RequestBody(required = false) TevglActivitySigninTrainee tevglActivitySigninTrainee) throws CreatorblueException {
	...
	public R update(@RequestBody(required = false) TevglActivitySigninTrainee tevglActivitySigninTrainee) throws CreatorblueException {
	    //ValidatorUtils.check(tevglActivitySigninTrainee);
	 ...
	 */
	@Override
	public R signIn(TevglActivitySigninTrainee tevglActivitySigninTrainee, String loginUserId) throws CreatorblueException {
		...
		tevglActivitySigninTrainee.setSignTime(DateUtils.getNowTimeStamp());
		...
		if (StrUtils.isEmpty(attachId)) {
			tsysAttachService.updateAttachForAdd(attachId, uuid, "18");
		}
		// 更新签到人数
	...
	private Integer toGetEmpiricalValue(String ctId, String signState) {
		...
		Integer empiricalValue = 0; // tevglEmpiricalSettingMapper.getEmpiricalValueByMap(map);
	...
	private boolean isInTimeRange(Integer limitTime, String createTime) {
		if (limitTime != null && StrUtils.isNotEmpty(createTime)) {
			Date date = DateTools.convertStringToDate(createTime);
	...
	public R setTraineeSignState(@RequestBody JSONObject jsonObject, String loginUserId) throws CreatorblueException {
		...
		Map<String, Object> map = new HashMap<>();
		List<String> traineeIds = (List<String>) jsonArray.stream().map(a -> a.toString()).collect(Collectors.toList());
		if (traineeIds != null) {
			...
			if (signinTraineeList == null || signinTraineeList.size() == 0) {
				...
				if (stIds.size() > 0) {
					...
					dataMap.put("signTime", DateUtils.getNowTimeStamp());
