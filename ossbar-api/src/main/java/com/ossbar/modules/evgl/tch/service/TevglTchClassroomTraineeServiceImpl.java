...
public class TevglTchClassroomTraineeServiceImpl implements TevglTchClassroomTraineeService {
	...
	public R joinTheClassroom(@RequestParam Map<String, Object> map, String loginUserId) throws CreatorblueException {
	 ...
	 */
	@Override
	public R deleteClassroomTrainee(Map<String, Object> map) throws CreatorblueException {
	...
	public R importTraineeBatch(String ctId, String classId, List<String> traineeIds) throws CreatorblueException {
	...
	public R viewTraineeBaseInfoForWeb(String ctId, String traineeId) {
		...
		params.put("traineeId", traineeId);
		// 当前学员课堂获取到的总经验值
		Integer currRoomEmpiricalValue = 0; // tevglTraineeEmpiricalValueLogMapper.sumEmpiricalValueByMap(params);
		// 课堂成员信息
		List<Map<String, Object>> classroomTraineeList = tevglTchClassroomTraineeMapper.selectListMapForWeb(params);
		if (classroomTraineeList == null || classroomTraineeList.size() == 0) {
			return R.error("未查询到相关数据");
		...
		// 博客点赞数
		Integer blogLikeNums = 0; // tevglForumBlogPostMapper.getTotalBlogLikes(traineeId);
		// 课堂表现
	...
	private Map<String, Object> getResourceInfo(TevglTchClassroom tevglTchClassroom, String traineeId) {
		...
		params.put("subjectId", subjectId);
		params.put("traineeId", traineeId);
		Integer emChapters = 0; // tevglEmpiricalLogChapterMapper.countRoomSubjectChapterNum(params);
	...
	public List<ClassroomTraineeVO> findClassroomTraineeList(Map<String, Object> map) {
		Query query = new Query(map);
		List<ClassroomTraineeVO> list = tevglTchClassroomTraineeMapper.findClassroomTraineeList(query);
		convertUtil.convertDict(list, "traineeSexName", "sex");
		return list;
	}
		...
		if (idList != null && idList.size() > 0) {
			for (int i = 0; i < idList.size(); i++) {
				if (!ids.contains(idList.get(i))) {
					throw new CreatorblueException(BizCodeEnume.ILLEGAL_OPERATION.getMsg());
				}
			}
		}
	}
